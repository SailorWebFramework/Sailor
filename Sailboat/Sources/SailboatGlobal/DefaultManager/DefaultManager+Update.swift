...
//                    node.build(child: makeList())
            ...
            for i in 0..<endRange {
                // TODO: #IMPORTANT CHECK if the custom pages are the same type then reuse the old page
                
                // iff node.children[i] && page.children[i] == custompage
                if node.children[i].compareTag(to: page.children[i]) {

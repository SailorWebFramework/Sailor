...
public final class HTMLStringNode: Renderable {
    ...
    /// Attributes collected via updateAttribute; a nil value is a bare boolean attribute
    ...
    public func updateAttribute(name: String, value: any AttributeValue) {
        ...
        // boolean attributes are added or removed, never written as "false"
        if let boolean = value as? BooleanAttribute {
            attributes.removeAll { $0.0 == name }
            if boolean.isPresent { attributes.append((name, nil)) }
            return
        }
